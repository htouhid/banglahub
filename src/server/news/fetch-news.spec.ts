...
it('requests NewsAPI with server-only header and Bangladesh query', async () => {
  ...
  expect(url.searchParams.get('q')).toBe('Bangladesh OR Dhaka');
...
it('filters invalid articles, sorts, limits results and ignores content', () => {
  ...
  expect(items).toHaveLength(15); expect(items[0].title).toBe('Headline 19');
